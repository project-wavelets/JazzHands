...
__all__ = [
    'phi_1', 'phi_2', 'phi_3', 'w_alpha', 'N_eff', 'function_inner_product',
    'S_matrix', 'inner_product_vector', 'coeffs', 'y_fit', 'V_x', 'V_y', 'WWZ'
]
...
def S_matrix(func_vals, ws):
    ...
    """
    S = np.array([[function_inner_product(f1, f2, ws) for f1 in func_vals]
                  for f2 in func_vals])
    return np.matrix(S)
...
def inner_product_vector(func_vals, ws, y):
    ...
    """
    return np.array(
        [[function_inner_product(func, y, ws) for func in func_vals]]).T
...
def V_x(f1_vals, ws, y):
    ...
    """
    return function_inner_product(y, y, ws) - np.power(
        function_inner_product(f1_vals, y, ws), 2.0)
...
def V_y(func_vals, f1_vals, ws, y):
    ...
    return function_inner_product(y_f, y_f, ws) - np.power(
        function_inner_product(f1_vals, y_f, ws), 2.0), y_a
...
def WWZ(func_list, f1, y, t, omega, tau, c=0.0125, exclude=True):
    ...
    return ((Neff - 3.0) * Vy) / (2.0 * (Vx - Vy)), np.sqrt(
        np.power(y_a_rows[1], 2.0) + np.power(y_a_rows[2], 2.0))
...
def MP_WWZ(func_list, f1, y, t, omegas, taus, c=0.0125, exclude=True, mp=True, n_processes=None):
    ...
    if mp:
        args = np.array([[func_list, f1, y, t, omega, tau, c, exclude]
                         for omega in omegas for tau in taus])
        ...
        with multiprocessing.Pool(processes=n_processes) as pool:
            results = pool.starmap(WWZ,
                                   args,
                                   chunksize=int(len(omegas) * len(taus) / 10))
        ...
        transform = np.array([[
            WWZ(func_list, f1, y, t, omega, tau, c, exclude) for tau in taus
        ] for omega in this_tqdm(omegas)])
        wwz = transform[:, :, 0].T
        wwa = transform[:, :, 1].T
